...
//随机放缩
void imgItem::setRandomRotation(double size)
{

    imgRecord* _old=getImgRecord();
      if(!_old)return;
      const int h = _old->get()->height(),
          w = _old->get()->width();
        int sx=w*rand()%2;
        int sy=h*rand()%2;
         resize(sx,sy);
...
void imgItem::mouseReleaseEvent(QMouseEvent *e)
{
   qDebug()<<"mouse_release";
}
void imgItem::mouseMoveEvent(QMouseEvent *e)
{
   qDebug()<<"mouse_move";
}
void imgItem::mouseDoubleCLickEvent(QMouseEvent *e)
{
   qDebug()<<"mouse_doubleclick";
}

void imgItem::keyPressEvent(QKeyEvent *e)
{
...
void imgItem::keyReleaseEvent(QKeyEvent *e)
{
   qDebug()<<"keyrelease";
}
